...
``SonoRo_swarm.py`` then passes then to ``RobotMove.py`` class, which moves the robot's wheel accorsingly to perform the selected behaviour.
...
When the dB SPL is above `critical` the threshold, the robot will move away from the sound source, while when it is above the `trigger` threshold, it will move towards it.
